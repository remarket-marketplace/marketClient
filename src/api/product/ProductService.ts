...
export const productService = {
  ...
  async updateProduct(productData: any, productId: string, uploadedImages: File[] = [], deletedImageIds: string[] = []) {
    try {
      const formData = new FormData()
  ...
  async approveProduct(productId: string) {
    try {
      const response = await httpClient.post('/products/approve', { product_id: productId })
      return response.status === 200
    }
    catch {
      return false
  ...
  async buyProduct(productId: string) {
    try {
      const response = await httpClient.post(`/products/buy`, { product_id: productId })
      return response.status === 200
    }
    catch {
      return false
  ...
  async getUserProductsByUsername(username: string): Promise<Product[]> {
    try {
      const response = await httpClient.get(`/products/user/by-username/${username}`)
      return response.data.map((product: any) => {
        const transformedProduct = {
          ...product,
          images: product.images.map((img: any) => ({
