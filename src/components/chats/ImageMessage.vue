...
const readStatusClass = computed(() => {
  if (!props.imageMessage || !isOwnMessage.value) return ''
  return props.imageMessage.is_read ? 'text-[rgb(var(--palette-sky-300))]' : 'text-[rgb(var(--palette-gray-300)/0.85)]'
})
...
<template>
  ...
  >
    <div v-if="senderLabel || forceShowSender" class="mb-1 flex items-center gap-2">
      <span class="rounded-full border border-[rgb(var(--palette-dark-600))] bg-[rgb(var(--palette-dark-700)/0.8)] px-2 py-0.5 text-[11px] font-semibold text-[rgb(var(--palette-gray-200))]">
      ...
      <div class="grid w-full gap-1.5" :class="imageGridClass">
        ...
        >
          ...
          <div
            ...
            class="absolute inset-0 z-10 flex items-center justify-center bg-[rgb(var(--palette-dark-700)/0.8)] px-2 text-center text-xs text-[rgb(var(--palette-gray-300))]"
          >
            {{ t('common.noImage') }}
          </div>
          <img
    ...
    <div class="mt-1 flex w-full items-center justify-end gap-2 text-xs text-[rgb(var(--palette-gray-300))]">
    ...
    >
      <div class="relative mx-auto flex h-full w-full max-w-7xl items-center justify-center" @click.stop>
        ...
        <div
          ...
          class="absolute inset-0 z-10 flex items-center justify-center px-4 text-center text-sm text-[rgb(var(--palette-gray-300))]"
        >
          {{ t('common.noImage') }}
        </div>
        <img
        ...
        <button
          type="button"
          class="absolute right-4 top-4 rounded-full bg-[rgb(var(--palette-black)/0.5)] p-2 text-[rgb(var(--palette-white))] transition-all duration-200 hover:bg-[rgb(var(--palette-black)/0.7)] hover:text-[rgb(var(--palette-gray-300))]"
          @click="closeImageModal"
        >
          <X class="h-6 w-6" />
        </button>
