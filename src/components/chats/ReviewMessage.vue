  ...
  <div class="my-2 flex w-full justify-center">
    <div class="message-compose-shell mx-auto flex w-full max-w-2xl min-w-0 flex-col overflow-hidden rounded-[26px] border border-[rgb(var(--palette-white)/0.1)] bg-background/85 px-4 py-3 backdrop-blur-xl supports-[backdrop-filter]:bg-background/70">
      <div class="mb-3 flex flex-wrap items-center justify-between gap-2 border-b border-[rgb(var(--palette-white)/0.08)] pb-2.5">
        <div class="min-w-0">
          <p class="text-[11px] font-semibold uppercase tracking-[0.14em] text-[rgb(var(--palette-blue-300)/0.9)]">
            {{ $t('pages.chats.newReview') }}
          </p>
          <p class="mt-1 text-sm font-medium text-[rgb(var(--palette-white))] break-words [overflow-wrap:anywhere]">
      ...
      <div class="flex flex-col gap-3">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-start justify-between gap-2 sm:gap-3">
              <div class="min-w-0 flex-1">
                <button
                  type="button"
                  class="mt-1 block min-w-0 text-left text-sm font-semibold text-[rgb(var(--palette-white))] transition hover:text-[rgb(var(--palette-blue-200))]"
                  @click="handleViewProduct"
                >
              ...
              <p class="text-sm font-semibold text-[rgb(var(--palette-emerald-300))]">
        ...
        <div class="flex items-center gap-1">
          <span
            ...
            :key="index"
            class="text-base"
            :class="isActive ? 'text-[rgb(var(--palette-blue-500))]' : 'text-[rgb(var(--palette-white)/0.15)]'"
          >
            ★
          </span>
        </div>
        ...
        <p
          ...
          :class="message.review.body ? 'text-[rgb(var(--palette-gray-200))]' : 'text-[rgb(var(--palette-gray-400))]'"
        >
          {{ message.review.body || $t('pages.chats.withoutReviewText') }}
        </p>

        <div v-if="timelineTimestamp" class="mt-3 flex items-center justify-end text-xs text-[rgb(var(--palette-gray-300))]">
