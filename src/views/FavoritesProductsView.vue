...
<template>
  <div class="w-full h-full flex flex-col items-center">
    ...
    <!-- Products list -->
    <div v-else class="w-full space-y-4 max-w-5xl border border-dark-600 rounded-lg overflow-hidden pt-2">
      ...
      <div v-if="products.length === 0" class="w-full text-center py-8 text-text-secondaryDark">
        {{ $t('common.noData') }}
      </div>
      ...
      <div v-else>
        <div v-for="product in products" :key="product.id"
          ...
          <FavoriteProductCard :product="product" :is-owner="false" @removed="onProductRemoved" />
        </div>
      </div>
    </div>
  </div>
