...
<template>
  <section v-if="product" class="no-scrollbar h-full max-w-7xl w-full flex flex-col items-start gap-6 overflow-scroll px-4 pb-36 text-white lg:flex-row lg:px-0">
    ...
    <div class="w-full border-1 border-dark-600 rounded-lg p-6 lg:flex-1 space-y-4">
      <div class="flex flex-col gap-4">
        <h1 class="truncate text-2xl text-white font-bold sm:text-3xl">
          {{ product.title }}
        </h1>
      ...
      <div class="whitespace-pre-line text-sm text-gray-300 leading-relaxed sm:text-base">
        {{ product.description || $t('product.descriptionMissing') }}
      </div>

      <!-- Мета информация, как в профиле -->
      <div class="border-t border-gray-700 pt-4 text-sm text-gray-400 space-y-3">
        <div class="flex items-center gap-2">
          <span class="text-white font-semibold">{{ $t('product.published') }}:</span>
          <span>{{ formatFullDate(product.created_at) }}</span>
        </div>
        <div class="flex items-center gap-2">
          <span class="text-white font-semibold">{{ $t('product.category') }}:</span>
          <span>{{ product.category?.name ?? $t('product.notSpecified') }}</span>
        </div>
        <div class="flex items-center gap-2">
          <span class="text-white font-semibold">{{ $t('product.status') }}:</span>
          <span
          ...
          <span
            ...
            class="rounded bg-yellow-600 px-2 py-1 text-white"
          >
          ...
          <span
            ...
            class="rounded bg-gray-600 px-2 py-1 text-white"
          >
            {{ $t('profile.purchased') }}
          </span>
        </div>
        ...
        <div class="flex cursor-pointer items-center gap-3 border-t border-gray-700 pt-2" @click="router.push(`/profile/${product.seller.username}`)">
          ...
          <div>
            ...
            <p class="text-xs text-gray-400">
              {{ $t('product.rating') }}: {{ product.seller.rating.toFixed(1) }}
            </p>
          </div>
        </div>
      </div>
      ...
      <div v-if="!product.is_sold" class="flex flex-col gap-3 border-t border-gray-700 pt-4 sm:flex-row">
        ...
        <template v-else>
          <button
            class="w-full rounded-lg bg-blue-600 px-6 py-3 text-base text-white font-semibold transition sm:w-auto hover:bg-blue-700"
    ...
    <Teleport to="body">
      ...
      >
        ...
        >
        <button
          class="absolute right-4 top-4 text-3xl text-white font-bold hover:text-blue-500"
    ...
    <ConfirmDelete
      ...
      :confirm-text="$t('product.deleteConfirm')"
      :cancel-text="$t('product.deleteCancel')"
