...
<template>
  <div class="w-full h-full overflow-scroll  lg:overflow-hidden pb-16 md:pb-0">
    <div v-if="actionError" class="mx-4 mt-4 bg-[rgb(var(--palette-red-500)/0.15)] border border-[rgb(var(--palette-red-500)/0.3)] text-[var(--text-danger-soft)] px-3 py-2 rounded-lg">
      {{ actionError }}
    </div>
    <div v-if="actionSuccess" class="mx-4 mt-2 bg-[rgb(var(--palette-green-500)/0.15)] border border-[rgb(var(--palette-green-500)/0.3)] text-[var(--text-success)] px-3 py-2 rounded-lg">
      {{ actionSuccess }}
    </div>
    <!-- Mobile header -->
    <div class="mb-6 lg:hidden px-4 pt-4">
      ...
      <div v-if="deal" class="flex items-center gap-3">
        ...
        <span class="text-xl font-bold text-[var(--text-success-strong)]">
        ...
        <div class="px-4 lg:px-0 lg:pb-6 space-y-6">
          ...
          <div class="hidden lg:flex items-center justify-between mb-6">
            ...
            <div v-if="deal" class="flex items-center gap-4">
              <DealStatusTag :deal-status="deal.status" />
              <span class="text-2xl font-bold text-[var(--text-title)]">
                {{ formatPrice(deal.price) }}
              </span>
            </div>
          </div>
          ...
          <div v-if="isLoading" class="flex items-center justify-center h-64">
            <Loader2 class="h-8 w-8 animate-spin text-[var(--text-link)]" />
            <span class="ml-3 text-lg text-[var(--text-muted)]">{{ $t('common.loading') }}</span>
          </div>
          ...
          <div v-else-if="errorMessage" class="rounded-xl border border-[rgb(var(--palette-red-500)/0.25)] bg-[rgb(var(--palette-red-500)/0.05)] p-4">
            <p class="text-[var(--text-danger)] text-center">{{ errorMessage }}</p>
          </div>
          ...
          <div v-else-if="deal" class="space-y-6">
            ...
            <div class="admin-surface-panel rounded-xl p-5">
              <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-semibold text-[var(--text-title)] flex items-center gap-2">
                  <Package class="w-5 h-5 text-[var(--text-link)]" />
              ...
              <div class="flex gap-4">
                ...
                <div class="flex-1 min-w-0">
                  <h3 class="text-xl font-semibold text-[var(--text-title)] mb-2 line-clamp-2 cursor-pointer"
                    @click="goToProduct(deal.product.id, deal.product.slug)">
                    {{ deal.product.title }}
                  </h3>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                    <div class="flex items-center gap-2 text-sm text-[var(--text-muted)]">
                      <span class="font-medium text-[var(--text-body)]">{{ $t('common.category') }}:</span>
                      <span>{{ deal.product.category.name }}</span>
                    </div>
                    <div class="flex items-center gap-2 text-sm text-[var(--text-muted)]">
                      <span class="font-medium text-[var(--text-body)]">{{ $t('common.quantity') }}:</span>
                      <span>{{ deal.product.count }} {{ $t('common.items') }}</span>
                    </div>
                    <div class="flex items-center gap-2 text-sm text-[var(--text-muted)]">
                      <span class="font-medium text-[var(--text-body)]">{{ $t('common.price') }}:</span>
                      <span>{{ formatPrice(deal.product.price) }}</span>
                    </div>
                    <div class="flex items-center gap-2 text-sm text-[var(--text-muted)]">
                      <span class="font-medium text-[var(--text-body)]">{{ $t('common.status') }}:</span>
                      <ProductStatusTag :product-status="deal.product.status" />
                    </div>
                  </div>

                  <p class="text-sm text-[var(--text-body)] line-clamp-3">
              ...
              <div v-if="deal.product.product_data_string" class="mt-4 pt-4 border-t border-[rgb(var(--palette-white)/0.08)]">
                <h4 class="text-sm font-medium text-[var(--text-body)] mb-2">{{ $t('pages.forms.createProduct.productData') }}:
                </h4>
                <pre
                  class="admin-surface-soft text-sm text-[var(--text-muted)] p-3 rounded-lg overflow-x-auto whitespace-pre-wrap font-mono">
{{ deal.product.product_data_string }}
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <!-- Seller -->
              <div class="admin-surface-panel rounded-xl p-5">
                <h3 class="text-lg font-semibold text-[var(--text-title)] flex items-center gap-2 mb-4">
                  <User class="w-5 h-5 text-[var(--text-success-strong)]" />
                  {{ $t('common.seller') }}
                </h3>
                ...
                <div class="flex items-center gap-3 mb-4">
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-1">
                      <span class="text-lg font-semibold text-[var(--text-title)] truncate cursor-pointer"
                        @click="goToProfile(deal.seller.username)">
                ...
                <div class="grid grid-cols-2 gap-3 text-sm">
                  <div class="space-y-1">
                    <span class="text-[var(--text-muted)]">{{ $t('common.memberSince') }}:</span>
                    <span class="text-[var(--text-title)] block">{{ formatDate(deal.seller.created_at.toString()) }}</span>
                  </div>
                  <div class="space-y-1">
                    <span class="text-[var(--text-muted)]">{{ $t('common.role') }}:</span>
                    <span class="text-[var(--text-title)] block">{{ deal.seller.role }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="admin-surface-panel rounded-xl p-5">
                <h3 class="text-lg font-semibold text-[var(--text-title)] flex items-center gap-2 mb-4">
                  <User class="w-5 h-5 text-[var(--text-link)]" />
                  {{ $t('common.buyer') }}
                </h3>
                ...
                <div class="grid grid-cols-2 gap-3 text-sm">
                  <div class="space-y-1">
                    <span class="text-[var(--text-muted)]">{{ $t('common.memberSince') }}:</span>
                    <span class="text-[var(--text-title)] block">{{ formatDate(deal.buyer.created_at.toString()) }}</span>
                  </div>
                  <div class="space-y-1">
                    <span class="text-[var(--text-muted)]">{{ $t('common.role') }}:</span>
                    <span class="text-[var(--text-title)] block">{{ deal.buyer.role }}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="admin-surface-panel rounded-xl p-5">
              <h3 class="text-lg font-semibold text-[var(--text-title)] flex items-center gap-2 mb-4">
                <CreditCard class="w-5 h-5 text-[var(--text-warning-strong)]" />
                {{ $t('pages.admin.dealPage.dealDetails') }}
              </h3>

              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div class="space-y-2">
                  <span class="text-sm text-[var(--text-muted)]">{{ $t('common.created') }}:</span>
                  <div class="flex items-center gap-2 text-[var(--text-title)]">
                ...
                <div class="space-y-2 space-x-2">
                  <span class="text-sm text-[var(--text-muted)]">{{ $t('common.status') }}:</span>
                  <DealStatusTag :deal-status="deal.status" />
                </div>
              </div>
            </div>
            ...
            <div v-if="deal.status === 'disputed' && deal.refusal_reason"
              ...
              <h3 class="text-lg font-semibold text-[var(--text-title)] flex items-center gap-2 mb-4">
                <Shield class="w-5 h-5 text-[var(--text-danger)]" />
                {{ $t('pages.admin.dealPage.reportInfo') }}
              </h3>

              <div class="space-y-4">
                <div>
                  <span class="text-sm text-[var(--text-muted)]">{{ $t('common.reason') }}:</span>
                  <p class="text-[var(--text-title)] mt-1 font-medium">
                ...
                <div v-if="deal.refusal_reason.refusal_reason_title === 'otherReason' && deal.refusal_reason.text">
                  <span class="text-sm text-[var(--text-muted)]">{{ $t('pages.admin.dealPage.customReason') }}:</span>
                  <div class="admin-surface-soft mt-2 p-3 rounded-lg">
                    <p class="text-[var(--text-body)] whitespace-pre-wrap">{{ deal.refusal_reason.text }}</p>
                  </div>
                </div>
          ...
          <div class="space-y-6 pb-6 lg:pb-0">
            ...
            <div v-if="deal" class="admin-surface-panel rounded-xl p-5 space-y-4">
              <h3 class="text-sm font-semibold text-[var(--text-title)] flex items-center gap-2">
                <Shield class="w-4 h-4 text-[var(--text-link)]" />
              ...
              <div class="space-y-3">
                ...
                <template v-else>
                  <div class="admin-surface-soft text-center py-3 px-4 rounded-lg text-[var(--text-body)]">
                    <div class="flex items-center justify-center gap-2">
                      <CheckCircle v-if="deal.status === 'completed'" class="w-5 h-5 text-[var(--text-success-strong)]" />
                      <Undo2 v-else-if="deal.status === 'refunded'" class="w-5 h-5 text-[var(--text-link)]" />
                      <XCircle v-else class="w-5 h-5 text-[var(--text-danger)]" />
                      <span>
                        {{
