...
<template>
  <div class="w-full h-full overflow-scroll  lg:overflow-hidden pb-16 md:pb-0">
    <div v-if="actionError" class="mx-4 mt-4 bg-[rgb(var(--palette-red-500)/0.15)] border border-[rgb(var(--palette-red-500)/0.3)] text-[rgb(var(--palette-red-100))] px-3 py-2 rounded-lg">
      {{ actionError }}
    </div>
    <div v-if="actionSuccess" class="mx-4 mt-2 bg-[rgb(var(--palette-green-500)/0.15)] border border-[rgb(var(--palette-green-500)/0.3)] text-[rgb(var(--palette-green-100))] px-3 py-2 rounded-lg">
      {{ actionSuccess }}
    </div>
    <!-- Mobile header -->
    <div class="mb-6 lg:hidden px-4 pt-4">
      <div class="flex items-center gap-3 mb-4">
        <button @click="goBack" class="admin-btn admin-btn-ghost admin-btn-icon">
          <ArrowLeft class="w-5 h-5 text-[rgb(var(--palette-white))]" />
        </button>
      ...
      <div v-if="deal" class="flex items-center gap-3">
        ...
        <span class="text-xl font-bold text-[rgb(var(--palette-green-400))]">
        ...
        <div class="px-4 lg:px-0 lg:pb-6 space-y-6">
          ...
          <div class="hidden lg:flex items-center justify-between mb-6">
            ...
            <div v-if="deal" class="flex items-center gap-4">
              <DealStatusTag :deal-status="deal.status" />
              <span class="text-2xl font-bold text-[rgb(var(--palette-white))]">
                {{ formatPrice(deal.price) }}
              </span>
            </div>
          </div>
          ...
          <div v-if="isLoading" class="flex items-center justify-center h-64">
            <Loader2 class="h-8 w-8 animate-spin text-[rgb(var(--palette-blue-500))]" />
            <span class="ml-3 text-lg text-[rgb(var(--palette-gray-400))]">{{ $t('common.loading') }}</span>
          </div>

          <!-- Error state -->
          <div v-else-if="errorMessage" class="rounded-xl border border-[rgb(var(--palette-red-500)/0.25)] bg-[rgb(var(--palette-red-500)/0.05)] p-4">
            <p class="text-[rgb(var(--palette-red-400))] text-center">{{ errorMessage }}</p>
          </div>
          ...
          <div v-else-if="deal" class="space-y-6">
            ...
            <div class="admin-surface-panel rounded-xl p-5">
              <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-semibold text-[rgb(var(--palette-white))] flex items-center gap-2">
                  <Package class="w-5 h-5 text-[rgb(var(--palette-blue-400))]" />
              ...
              <div class="flex gap-4">
                ...
                <div class="flex-shrink-0">
                  <img :src="getProductImageUrl()" :alt="deal.product.title"
                    class="w-24 h-24 rounded-lg object-cover border border-[rgb(var(--palette-white)/0.1)] cursor-pointer"
                    @click="goToProduct(deal.product.id, deal.product.slug)" />
                </div>

                <!-- Product details -->
                <div class="flex-1 min-w-0">
                  <h3 class="text-xl font-semibold text-[rgb(var(--palette-white))] mb-2 line-clamp-2 cursor-pointer"
                    @click="goToProduct(deal.product.id, deal.product.slug)">
                    {{ deal.product.title }}
                  </h3>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                    <div class="flex items-center gap-2 text-sm text-[rgb(var(--palette-gray-400))]">
                      <span class="font-medium text-[rgb(var(--palette-gray-300))]">{{ $t('common.category') }}:</span>
                      <span>{{ deal.product.category.name }}</span>
                    </div>
                    <div class="flex items-center gap-2 text-sm text-[rgb(var(--palette-gray-400))]">
                      <span class="font-medium text-[rgb(var(--palette-gray-300))]">{{ $t('common.quantity') }}:</span>
                      <span>{{ deal.product.count }} {{ $t('common.items') }}</span>
                    </div>
                    <div class="flex items-center gap-2 text-sm text-[rgb(var(--palette-gray-400))]">
                      <span class="font-medium text-[rgb(var(--palette-gray-300))]">{{ $t('common.price') }}:</span>
                      <span>{{ formatPrice(deal.product.price) }}</span>
                    </div>
                    <div class="flex items-center gap-2 text-sm text-[rgb(var(--palette-gray-400))]">
                      <span class="font-medium text-[rgb(var(--palette-gray-300))]">{{ $t('common.status') }}:</span>
                      <ProductStatusTag :product-status="deal.product.status" />
                    </div>
                  </div>

                  <p class="text-sm text-[rgb(var(--palette-gray-300))] line-clamp-3">
              ...
              <!-- Product data -->
              <div v-if="deal.product.product_data_string" class="mt-4 pt-4 border-t border-[rgb(var(--palette-white)/0.08)]">
                <h4 class="text-sm font-medium text-[rgb(var(--palette-gray-300))] mb-2">{{ $t('pages.forms.createProduct.productData') }}:
                </h4>
                <pre
                  class="admin-surface-soft text-sm text-[rgb(var(--palette-gray-400))] p-3 rounded-lg overflow-x-auto whitespace-pre-wrap font-mono">
{{ deal.product.product_data_string }}
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <!-- Seller -->
              <div class="admin-surface-panel rounded-xl p-5">
                <h3 class="text-lg font-semibold text-[rgb(var(--palette-white))] flex items-center gap-2 mb-4">
                  <User class="w-5 h-5 text-[rgb(var(--palette-green-400))]" />
                  {{ $t('common.seller') }}
                </h3>
                ...
                <div class="flex items-center gap-3 mb-4">
                  <UserAvatar
                    ...
                    class="w-14 h-14 rounded-full object-cover border-2 border-[rgb(var(--palette-green-500)/0.3)] cursor-pointer"
                    @click="goToProfile(deal.seller.username)"
                  />
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-1">
                      <span class="text-lg font-semibold text-[rgb(var(--palette-white))] truncate cursor-pointer"
                        @click="goToProfile(deal.seller.username)">
                ...
                <div class="grid grid-cols-2 gap-3 text-sm">
                  <div class="space-y-1">
                    <span class="text-[rgb(var(--palette-gray-400))]">{{ $t('common.memberSince') }}:</span>
                    <span class="text-[rgb(var(--palette-white))] block">{{ formatDate(deal.seller.created_at.toString()) }}</span>
                  </div>
                  <div class="space-y-1">
                    <span class="text-[rgb(var(--palette-gray-400))]">{{ $t('common.role') }}:</span>
                    <span class="text-[rgb(var(--palette-white))] block">{{ deal.seller.role }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="admin-surface-panel rounded-xl p-5">
                <h3 class="text-lg font-semibold text-[rgb(var(--palette-white))] flex items-center gap-2 mb-4">
                  <User class="w-5 h-5 text-[rgb(var(--palette-blue-400))]" />
                  {{ $t('common.buyer') }}
                </h3>
                ...
                <div class="flex items-center gap-3 mb-4">
                  <UserAvatar
                    ...
                    class="w-14 h-14 rounded-full object-cover border-2 border-[rgb(var(--palette-blue-500)/0.3)] cursor-pointer"
                ...
                <div class="grid grid-cols-2 gap-3 text-sm">
                  <div class="space-y-1">
                    <span class="text-[rgb(var(--palette-gray-400))]">{{ $t('common.memberSince') }}:</span>
                    <span class="text-[rgb(var(--palette-white))] block">{{ formatDate(deal.buyer.created_at.toString()) }}</span>
                  </div>
                  <div class="space-y-1">
                    <span class="text-[rgb(var(--palette-gray-400))]">{{ $t('common.role') }}:</span>
                    <span class="text-[rgb(var(--palette-white))] block">{{ deal.buyer.role }}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="admin-surface-panel rounded-xl p-5">
              <h3 class="text-lg font-semibold text-[rgb(var(--palette-white))] flex items-center gap-2 mb-4">
                <CreditCard class="w-5 h-5 text-[rgb(var(--palette-yellow-400))]" />
                {{ $t('pages.admin.dealPage.dealDetails') }}
              </h3>

              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div class="space-y-2">
                  <span class="text-sm text-[rgb(var(--palette-gray-400))]">{{ $t('common.created') }}:</span>
                  <div class="flex items-center gap-2 text-[rgb(var(--palette-white))]">
                    <Calendar class="w-4 h-4 text-[rgb(var(--palette-gray-400))]" />
                    <span>{{ formatDate(deal.created_at) }}</span>
                  </div>
                </div>

                <div class="space-y-2 space-x-2">
                  <span class="text-sm text-[rgb(var(--palette-gray-400))]">{{ $t('common.status') }}:</span>
                  <DealStatusTag :deal-status="deal.status" />
                </div>
              </div>
            </div>
            ...
            <div v-if="deal.status === 'disputed' && deal.refusal_reason"
              class="rounded-xl border border-[rgb(var(--palette-red-500)/0.25)] bg-[rgb(var(--palette-red-500)/0.04)] p-5">
              <h3 class="text-lg font-semibold text-[rgb(var(--palette-white))] flex items-center gap-2 mb-4">
                <Shield class="w-5 h-5 text-[rgb(var(--palette-red-400))]" />
                {{ $t('pages.admin.dealPage.reportInfo') }}
              </h3>

              <div class="space-y-4">
                <div>
                  <span class="text-sm text-[rgb(var(--palette-gray-400))]">{{ $t('common.reason') }}:</span>
                  <p class="text-[rgb(var(--palette-white))] mt-1 font-medium">
                ...
                <div v-if="deal.refusal_reason.refusal_reason_title === 'otherReason' && deal.refusal_reason.text">
                  <span class="text-sm text-[rgb(var(--palette-gray-400))]">{{ $t('pages.admin.dealPage.customReason') }}:</span>
                  <div class="admin-surface-soft mt-2 p-3 rounded-lg">
                    <p class="text-[rgb(var(--palette-gray-300))] whitespace-pre-wrap">{{ deal.refusal_reason.text }}</p>
                  </div>
                </div>
      ...
      <div
        class="lg:w-96 lg:flex-shrink-0 lg:sticky lg:top-0 lg:h-full lg:border-l border-[rgb(var(--palette-white)/0.08)] px-4 lg:px-0 lg:pt-6 lg:pl-6">
          ...
          <div class="space-y-6 pb-6 lg:pb-0">
            ...
            <div v-if="deal" class="admin-surface-panel rounded-xl p-5 space-y-4">
              <h3 class="text-sm font-semibold text-[rgb(var(--palette-white))] flex items-center gap-2">
                <Shield class="w-4 h-4 text-[rgb(var(--palette-blue-400))]" />
                {{ $t('pages.admin.dealPage.adminActions') }}
              </h3>

              <!-- Actions based on deal status -->
              <div class="space-y-3">
                <div class="space-y-2 pb-2 border-b border-[rgb(var(--palette-white)/0.08)]">
                  <CustomSelect
                ...
                <template v-else>
                  <div class="admin-surface-soft text-center py-3 px-4 rounded-lg text-[rgb(var(--palette-gray-300))]">
                    <div class="flex items-center justify-center gap-2">
                      <CheckCircle v-if="deal.status === 'completed'" class="w-5 h-5 text-[rgb(var(--palette-green-400))]" />
                      <Undo2 v-else-if="deal.status === 'refunded'" class="w-5 h-5 text-[rgb(var(--palette-purple-400))]" />
                      <XCircle v-else class="w-5 h-5 text-[rgb(var(--palette-red-400))]" />
                      <span>
                        {{
