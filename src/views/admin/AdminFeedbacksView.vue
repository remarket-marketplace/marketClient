  ...
  <section class="h-full w-full flex flex-col gap-3 sm:gap-6 overflow-hidden pt-3 md:pt-4">
    ...
    <div class="flex-1 overflow-hidden">
      ...
      <div v-else-if="errorMessage" class="flex items-center justify-center h-32">
        <p class="text-[rgb(var(--palette-red-400))] text-sm sm:text-base">{{ errorMessage }}</p>
      </div>

      <div v-else-if="feedbacks.length === 0" class="h-full w-full flex items-center justify-center">
        <div class="text-center px-4">
          <MessageSquareText class="h-8 w-8 text-[rgb(var(--palette-gray-500))] mx-auto mb-3" />
          <p class="text-mainText font-semibold">{{ $t('pages.admin.feedbackPage.emptyTitle') }}</p>
          <p class="text-sm text-[rgb(var(--palette-gray-400))] mt-2">{{ $t('pages.admin.feedbackPage.emptyHint') }}</p>
        </div>
      </div>
          ...
          <div class="flex items-start justify-between gap-3">
            <div class="flex items-center gap-3 min-w-0">
              ...
              <div class="min-w-0">
                <button
                  type="button"
                  class="text-sm sm:text-base font-semibold text-mainText hover:text-[rgb(var(--palette-blue-400))] truncate max-w-[220px] text-left"
                  @click="openProfile(feedback.user.username)"
                >
                  {{ feedback.user.username }}
                </button>
                <p class="text-xs text-[rgb(var(--palette-gray-500))] mt-0.5">
                  {{ formatDate(feedback.created_at) }}
                </p>
              </div>
            </div>

            <div class="inline-flex items-center gap-1 text-xs text-[rgb(var(--palette-gray-400))]">
              <Images class="h-3.5 w-3.5" />
              <span>{{ $t('pages.admin.feedbackPage.imagesCount', { count: feedback.images_count }) }}</span>
            </div>
          </div>

          <p class="mt-3 text-sm text-[rgb(var(--palette-gray-200))] whitespace-pre-wrap break-words line-clamp-4">
