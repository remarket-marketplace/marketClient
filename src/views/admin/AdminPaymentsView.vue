...
function getStatusBadgeClass(status: PaymentStatus): string {
  const map: Record<PaymentStatus, string> = {
    PENDING: 'border-[rgb(var(--palette-amber-500)/0.4)] bg-[rgb(var(--palette-amber-500)/0.15)] text-[rgb(var(--palette-amber-200))]',
    CONFIRMED: 'border-[rgb(var(--palette-emerald-500)/0.4)] bg-[rgb(var(--palette-emerald-500)/0.15)] text-[rgb(var(--palette-emerald-200))]',
    CANCELED: 'border-[rgb(var(--palette-gray-500)/0.4)] bg-[rgb(var(--palette-gray-500)/0.15)] text-[rgb(var(--palette-gray-200))]',
    CHARGEBACKED: 'border-[rgb(var(--palette-rose-500)/0.4)] bg-[rgb(var(--palette-rose-500)/0.15)] text-[rgb(var(--palette-rose-200))]',
  }
  return map[status]
}
  ...
  <section class="h-full w-full flex flex-col gap-4 sm:gap-6 overflow-hidden pt-3 md:pt-4">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
      <div class="flex items-center gap-2">
        ...
        <div>
          ...
          <p class="text-xs sm:text-sm text-[rgb(var(--palette-gray-400))]">{{ $t('pages.admin.paymentsPage.subtitle') }}</p>
        </div>
      </div>
    ...
    <div class="flex-1 overflow-hidden">
      ...
      <div v-else-if="payments.length === 0" class="flex h-32 items-center justify-center">
        <p class="text-[rgb(var(--palette-gray-400))]">{{ $t('pages.admin.paymentsPage.empty') }}</p>
      </div>
        ...
        >
          <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex flex-wrap items-center gap-2">
              ...
              <span class="text-xs text-[rgb(var(--palette-gray-500))]">{{ $t('pages.admin.paymentsPage.paymentId') }}: {{ payment.id }}</span>
            </div>
          ...
          <div class="mt-3 grid grid-cols-1 gap-2 text-sm text-[rgb(var(--palette-gray-200))] sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div>
              <span class="text-[rgb(var(--palette-gray-400))]">{{ $t('pages.admin.paymentsPage.createdAt') }}:</span>
              <span class="ml-1 font-medium">{{ formatDate(payment.created_at) }}</span>
            </div>

            <div>
              <span class="text-[rgb(var(--palette-gray-400))]">{{ $t('pages.admin.paymentsPage.confirmedAt') }}:</span>
              <span class="ml-1 font-medium">{{ formatDate(payment.confirmed_at) }}</span>
            </div>

            <div>
              <span class="text-[rgb(var(--palette-gray-400))]">{{ $t('pages.admin.paymentsPage.updatedAt') }}:</span>
              <span class="ml-1 font-medium">{{ formatDate(payment.updated_at) }}</span>
            </div>

            <div>
              <span class="text-[rgb(var(--palette-gray-400))]">{{ $t('pages.admin.paymentsPage.provider') }}:</span>
              <span class="ml-1 font-medium">{{ payment.provider }}</span>
            </div>

            <div>
              <span class="text-[rgb(var(--palette-gray-400))]">{{ $t('pages.admin.paymentsPage.transactionId') }}:</span>
